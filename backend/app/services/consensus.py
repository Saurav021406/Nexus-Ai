"""Hybrid Consensus Engine.
   ...
   |    |    |
   v    v    v
 Groq Nemotron MiniMax-M3  <- called in PARALLEL in full consensus mode
   |    |    |                (or fast-path cascading in tier="fast" mode)
   +----+----+
        v
 Hybrid Consensus Engine
        |
...
Supports two tiers:
- tier="fast": Single fast model (Groq) with fallback cascade (~300ms latency, 1/3 token cost).
- tier="full": Full 3-way parallel execution with weighted ranking & contradiction scoring.
...
# ---------------------------------------------------------------------------
# Fast Tier: single-model fast cascade
# ---------------------------------------------------------------------------

def _query_fast_tier(prompt: str, temperature: float, max_tokens: int) -> ConsensusResult:
    """Fast-path tier: queries Groq first (~300ms), falling back to NVIDIA/MiniMax
    only if Groq encounters a failure."""
    order = ["groq", "nvidia", "minimax"]
    errors: list[str] = []

    for name in order:
        fn = _PROVIDERS[name]
        try:
            text = _call_with_retry(fn, prompt, temperature, max_tokens, retries=1)
            model_ans = ModelAnswer(name=name, weight=MODEL_WEIGHTS[name], text=text)
            return ConsensusResult(
                ...
                confidence=0.92,
                models_used=[name],
                synthesis_notes=[f"Fast tier response generated by {name}."],
            )
        except Exception as e:
            errors.append(f"{name}: {e}")
            continue

    raise RuntimeError(f"All fast-tier model providers failed. {' | '.join(errors)}")
...
def confidence_scoring(
    ...
    responed (fewer responses = less cross-checking = lower confidence).
...
def get_consensus(
    ...
    *,
    ...
    """Runs the Hybrid Consensus Engine pipeline:

        if tier == "fast":
            runs single-provider cascade (Groq -> NVIDIA -> MiniMax)
        else (tier == "full"):
    ...
    if tier == "fast":
        return _query_fast_tier(prompt, temperature, max_tokens)
...
def get_consensus_json(
    ...
    *,
    ...
    result = get_consensus(prompt, temperature=temperature, max_tokens=max_tokens, tier=tier)
