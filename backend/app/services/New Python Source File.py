...
and rescores them against the query using a cross-encoder / reranker model.
"""
...
# Optional lightweight flashrank support
try:
...
def _get_ranker() -> Any:
    ...
    if _ranker is None and _HAS_FLASHRANK:
        # Nano/Mini cross-encoder model (~4MB), runs in milliseconds on CPU
    ...
    if ranker:
        try:
            ...
            for r in results[:top_k]:
                ...
                chunk_copy["rerank_score"] = float(r.get("score", 0.0))
                reranked.append(chunk_copy)
            return reranked
        except Exception:
            # Fallback to lexical combination on any runtime error
            pass

    # Path B: Robust Fallback (existing similarity score + token overlap)
    scored = []
    for c in chunks:
